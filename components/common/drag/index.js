...
Component({
	...
	data: {
		// 删除位置的高度===》realBottomSize
		oldPosition: {},										// 缓存移动前的图片信息
		deleteHeight: 0,
		/* 未渲染数据 */
		inDelete: false,										// 是否再底部的删除区域内
	...
	methods: {
		...
		longPress(e) {
			...
			this.setData({
				dragging: true,
				oldPosition: {...this.data.list[index]}
			});
		...
		touchMove(e) {
			...
			let {pageMetaSupport, windowHeight, realTopSize, realBottomSize, itemDom, itemWrapDom, preStartKey, columns, rows, deleteHeight} = this.data,
		...
		// touchEnd(e) {
		// 	if (!this.data.dragging) return;
		// 	let {windowHeight, deleteHeight,cur,list} = this.data,
		// 	{ clientY: currentClientY} = e.changedTouches[0];
		// 	if (currentClientY > windowHeight - deleteHeight) {

		// 		this.setData({
		// 			newPosition: {...this.data.list[cur]},
		// 			inDelete: true
		// 		})
		// 		// 晚点把动画效果去掉直接删除试下 itemTransition

		// 		// let paramList = list.filter((_,index) => index !== cur).map((item,index) => {
		// 		// 	item.sortKey = index;
		// 		// 	return item
		// 		// })
		// 		// console.log((cur % this.data.columns) * this.data.itemDom.width,Math.floor(cur / this.data.columns) * 100 * this.data.itemDom.width,"===>jjjj");
		// 		let updateList = (data, cutItem, vibrate = true) => {
		// 			let {platform} = this.data;

		// 			let list = data.map((item, index) => {
		// 				if(index >= cutItem.sortKey){
		...
		// 		// this.sort(startKey, list.length-2)

		// 		// let newList = list.slice(0,list.length-1)
		// 		// console.log(list,"====>lisst")
		// 		// console.log(list.splice(0,list.length-2),"===>front")
		// 		// console.log(list.splice(list.length-1),"===>back")
		// 		// let newList = list.filter(item => item.sortKey !== cur)
		// 		let cutItem = list.splice(cur,1)
		// 		this.updateList(list);
		// 		// this.setData({
		// 		// 	inDelete: false,
		// 		// 	tranX: (cur % this.data.columns) * this.data.itemDom.width, 
		// 		// 	tranY: Math.floor(cur / this.data.columns) * this.data.itemDom.width, 
		// 		// })
		// 	}
		// 	this.triggerCustomEvent(list, "sortend");
		// 	this.clearData();
		// },
		touchEnd(e) {
			if (!this.data.dragging) return;
			...
			if (currentClientY > windowHeight - deleteHeight) {
				...
				let updateList = (data, cutItem, vibrate = true) => {
					...
					let affterDeletelist = data.map((item, index) => {
						let newSortKey = item.sortKey;
						if(item.sortKey > cutItem.sortKey){
							newSortKey = newSortKey - 1
						}
						item.tranX = `${(newSortKey % this.data.columns) * 100}%`;
						item.tranY = `${Math.floor(newSortKey / this.data.columns) * 100}%`;
						item.sortKey = newSortKey;
						return item;
					});
					this.clearData();
					this.setData({
						list: affterDeletelist,
						inDelete: false,
						oldPosition: {},
					});
					setTimeout(() => {
				...
				let startKey = parseInt(e.currentTarget.dataset.key);
				let cutItem = list.splice(cur,1).pop()
				updateList(list,cutItem);
				return
			}
			this.triggerCustomEvent(this.data.list, "sortend");
			console.log(this.data.list, "=====>hahahhahah");
			this.clearData();
		},
		/**
		...
		sort(startKey, endKey) {
			...
			console.log(this.data.list,"====>sort方法");
		...
		// updateList(data, vibrate = true) {
			// 	let {platform,inDelete,newPosition} = this.data;
			
			// 	let list = data.map((item, index) => {
				// 		let newItem = {}
				// 		if(inDelete && index >= newPosition.sortKey){
					// 			if(index == newPosition.sortKey){
						// 				newItem = {...item, tranX: newPosition.tranX, tranY: newPosition.tranY }
						// 			}else {
							// 				newItem = {...item, tranX: data[index-1].tranX, tranY: data[index-1].tranY }
							// 			}
							// 		}else{
								// 			newItem = {...item}
								// 			newItem.tranX = `${(item.sortKey % this.data.columns) * 100}%`;
								// 			newItem.tranY = `${Math.floor(item.sortKey / this.data.columns) * 100}%`;
								// 		}
								// 		return newItem;
								// 	});
								// 	if(inDelete){
	...
	ready() {
		...
		let remScale = (windowWidth || 375) / 375,
				realBottomSize = this.data.bottomSize * remScale / 2;
		this.setData({deleteHeight:realBottomSize})
		console.log(realBottomSize);
	}
});