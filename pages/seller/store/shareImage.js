// pages/seller/store/shareImage.js
...
Page({
  ...
  afterRead(event) {
    ...
    wx.redirectTo({
      url: '/pages/common/cropImage/index?width=500&height=400&from_url=/pages/seller/store/shareImage&id=' + this.data.id + '&url=' + this.data.url,
    })
    // const that = this
    // wx.uploadFile({
    //   url: baseUrl + 'upload',
  ...
  submit() {
    ...
    app.globalFun.http.get('store/edit', {
      share_image: this.data.headList[0].id || ''
    }, (res) => {
      if (res.code === 0) {
        wx.showToast({
          title: '保存成功',
